...
use shotover_transforms::{
    ChainResponse, Messages, TopicHolder, Transform, TransformsFromConfig, Wrapper,
};
...
impl Debug for Protect {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Protect")
            .field("name", &self.name)
            .field("keyspace_table_columns", &self.keyspace_table_columns)
...
mod protect_transform_tests {
    ...
    use crate::protect::key_management::KeyManagerConfig;
    ...
    async fn test_protect_transform() -> Result<(), Box<dyn Error>> {
        ...
        //
        // let mut wrapper = Messages::new_single_query(QueryMessage {
        ...
        // }, true, RawFrame::NONE);
        //
        // let transforms: Vec<Transforms> = vec![Box::new(Null::new())];
        //
        // let mut chain = TransformChain::new(transforms, String::from("test_chain"));
        //
        // wrapper.reset(chain.get_inner_chain_refs());
        //
        // if let Box::new(mut protect) = protect_t.get_source(&t_holder).await? {
