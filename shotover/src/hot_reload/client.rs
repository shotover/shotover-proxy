...
impl HotReloadClient {
    ...
    pub async fn request_shutdown_old_instance(&self, duration_secs: u64) -> Result<()> {
        info!(
            "Hot reload CLIENT requesting gradual shutdown of old shotover at: {} with duration {}s",
            self.socket_path, duration_secs
        );
