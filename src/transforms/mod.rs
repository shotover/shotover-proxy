...
impl TransformsConfig {
    pub async fn get_transforms(&self, topics: &TopicHolder, logger: &Logger) -> Result<Transforms, ConfigError> {
        match self {
...
pub async fn build_chain_from_config(name: String, transform_configs: &Vec<TransformsConfig>, topics: &TopicHolder, logger: &Logger) -> Result<TransformChain, ConfigError> {
