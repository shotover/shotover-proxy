...
use cql3_parser::{
    cassandra_ast::{CassandraAST, CassandraParser},
...
fn is_reading_table_and_column(
    query: &str,
    search_keyspace: &str,
    search_table: &str,
    search_column: &str,
    ...
    // skip past parsing the columns until we know the table
...
pub(crate) fn seek_read_table<'a>(_bytes: &'a [u8], query: &'a str) -> Option<&'a str> {
    Some("yes")

    // let opcode = Opcode::try_from(bytes[4]).unwrap();
    // match opcode {
    //     Opcode::Query => {
    //         let _body_len = try_i32_from_bytes(&bytes[5..9]).unwrap() as usize;
    //         // parse the query as a string
...
#[test]
fn test_peek_read_table() {
    let result = is_reading_table_and_column(
        "SELECT * FROM my_keyspace.mytable",
        "system",
        "peers_v2",
        "native_port",
    );
    ...
    let result1 = is_reading_table_and_column(
