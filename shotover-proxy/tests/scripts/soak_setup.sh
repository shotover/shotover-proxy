...
docker-compose -f $SCRIPT_DIR/../../tests/test-configs/redis-cluster/docker-compose.yaml up -d
echo "Getting ready to run proxy"
sleep 5
echo "Running shotover"

/usr/bin/time -v -o $SCRIPT_DIR/../../tests/soaktest-`date +%s` $SCRIPT_DIR/../../target/release/shotover-proxy --config-file $SCRIPT_DIR/../../config/config.yaml --topology-file $SCRIPT_DIR/../../tests/test-configs/redis-cluster/topology.yaml
