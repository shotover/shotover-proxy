...
function defer {
	docker-compose -f $SCRIPT_DIR/../../example-configs/cassandra-cluster/docker-compose.yaml down
}

trap defer EXIT

docker-compose -f $SCRIPT_DIR/../../example-configs/cassandra-cluster/docker-compose.yaml up
