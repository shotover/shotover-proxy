...
pub async fn test_trigger_transform_failure_driver(client: &Client) {
    assert_eq!(
        ...
        format!(
            "ERR Internal shotover (or custom transform) bug: Chain failed to send and/or receive messages, the connection will now be closed.  Caused by:     0: valkey-sink transform failed (chain: valkey)     1: Failed to connect to destination 127.0.0.1:1111     2: Connection refused (os error {CONNECTION_REFUSED_OS_ERROR})"
        )
    );
}
...
pub async fn test_trigger_transform_failure_raw() {
    ...
    assert_eq!(
        ...
        ValkeyFrame::Error(format!("ERR Internal shotover (or custom transform) bug: Chain failed to send and/or receive messages, the connection will now be closed.  Caused by:     0: valkey-sink transform failed (chain: valkey)     1: Failed to connect to destination 127.0.0.1:1111     2: Connection refused (os error {CONNECTION_REFUSED_OS_ERROR})").into())
    );
